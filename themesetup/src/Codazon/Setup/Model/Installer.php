...
class Installer extends \Magento\Setup\Model\Installer
{
    ...
    public function install($request)
    {
        ...
        $request['db'] = [];
        $request['db']['host'] = $data['db']['connection']['default']['host'];
        $request['db']['name'] = $data['db']['connection']['default']['dbname'];
        $request['db']['user'] = $data['db']['connection']['default']['username'];
        $request['db']['password'] = $data['db']['connection']['default']['password'];
        $request['db']['tablePrefix'] = $data['db']['table_prefix'];
        
	$script[] = ['Disabling caches:', 'disableCaches', []];
       ...
       // $script[] = ['Installing admin user...', 'installAdminUser', [$request]];
        ...
        $estimatedModules = $this->createModulesConfig($request, false);
        $total = count($script) + 4 * count(array_filter($estimatedModules));
        $this->progress = new \Magento\Setup\Model\Installer\Progress($total, 0);
    ...
    private function disableCaches()
    {
        ...
        $enabledTypes = $cacheManager->setEnabled($types, false);
    ...
    public function checkExtensions()
    {
        ...
        if ($phpExtensionsCheckResult['responseType'] === \Magento\Setup\Controller\ResponseTypeInterface::RESPONSE_TYPE_ERROR
    ...
    private function generateListOfModuleContext($resource, $type)
    {
        ...
        foreach ($this->moduleList->getNames() as $moduleName) {
            ...
            if ($dbVer !== false) {
                $moduleContextList[$moduleName] = new \Magento\Setup\Model\ModuleContext($dbVer);
            } else {
                $moduleContextList[$moduleName] = new \Magento\Setup\Model\ModuleContext('');
            }
        }
